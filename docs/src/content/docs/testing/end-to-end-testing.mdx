...
Before running E2E tests with Maestro, you need to ensure the following:

### Android Setup

For Android testing, you need:

1. **Android Debug Bridge (ADB)** - Maestro requires ADB to connect to Android devices/emulators.

   - **Windows**: Download [Android SDK Platform Tools](https://developer.android.com/tools/releases/platform-tools) and extract it. Add the `platform-tools` directory to your system PATH.
   - **macOS/Linux**: Install via package manager or download from [Android SDK Platform Tools](https://developer.android.com/tools/releases/platform-tools).
...
2. **Android Emulator or Physical Device**
   ...
   This should list your connected device(s). If you see "0 devices connected", ensure your emulator is running or your physical device is connected and USB debugging is enabled.

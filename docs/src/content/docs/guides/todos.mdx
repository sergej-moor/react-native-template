...
- **Modern UX**: Swipe-to-delete gestures, floating action button, and segmented filters
...
│
└── components/todos/
    ├── todo-item.tsx         # Individual todo with swipe-to-delete
    ├── todo-list.tsx         # FlashList implementation
    ├── todo-form.tsx         # Add/edit form with validation
    ├── todo-filters.tsx      # Segmented control filters
    ├── todo-header.tsx       # Progress bar and clear button
    └── empty-state.tsx       # Empty state views
```

### UX Features

The todo app implements modern mobile UX patterns:

#### Swipe-to-Delete

- **Gesture**: Swipe left on a todo item past the threshold and release to delete
- **Implementation**: Uses `react-native-gesture-handler`'s `Swipeable` component
- **Visual Feedback**: Red background with trash icon appears while swiping
- **Auto-Delete**: Releasing the swipe automatically triggers deletion (like Spotify, iOS Mail)
- **Smooth Animation**: Native-feeling gesture with proper physics
...
- **Gesture**: Tap anywhere on the todo item to toggle completion
- **Visual Feedback**: Active state provides instant feedback
- **Accessibility**: Full support for screen readers

#### Floating Action Button (FAB)

- **Position**: Fixed at bottom-right corner
- **Action**: Opens a bottom sheet modal for adding todos
- **Visual**: Clean, modern design with shadow

#### Segmented Filters

- **Style**: iOS-style segmented control
- **Filters**: All / Active / Completed
- **Visual**: Smooth transitions between states
